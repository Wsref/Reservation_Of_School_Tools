...
class ReservationMaterielController extends Controller
{
    public function index(){
        ...
        $reservationPasse = Reservationm::all();
    ...
    public function get_data_reserv(){
        ...
        foreach ($reservations as $reserv) {
            ...
            $reservData[] = [
                ...
                'end' =>  $reserv->date_reserve_end . ' '. $reserv->time_reserve_end,
                'color' => $color,
    ...
    public function demandes_reserve(Request $request){
        ...
        $demandes = Reservationm::where('valide','=',-1)->orderBy('date_reserve')->paginate(15);

        foreach($demandes as $data){
            $output.='<tr>';
            $output.='<td>'.$data->users->prenom.' '.$data->users->nom.'('.$data->materiels->name.' | '.$data->date_reserve.')'.'</td>'.
    ...
    public function demandes_reserve_after(Request $request){
        ...
        $demande = Reservationm::where('id','=',$request->resId)->first();
